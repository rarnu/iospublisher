...
type
  ...
  private
    ...
    FTxtUKey: TD2HudTextBox;
    FTxtApiKey: TD2HudTextBox;
  public

  end;

var
  FormPgyer: TFormPgyer;
...
procedure TFormPgyer.FormClose(Sender: TObject; var CloseAction: TCloseAction);
begin
  ioscfg.PgyerProvisionKey:= FTxtProvisionKey.Text;
  ioscfg.PgyerProvisionValue:= FTxtProvisionValue.Text;
  ioscfg.PgyerCertificate:= FTxtCertificate.Text;
  ioscfg.PgyerTeamId:= FTxtTeamId.Text;
  ioscfg.PgyerAutoSign:= FChkAutoSign.IsChecked;
  ioscfg.PgyerUKey:= FTxtUKey.Text;
  ioscfg.PgyerApiKey:= FTxtApiKey.Text;
end;

procedure TFormPgyer.FormCreate(Sender: TObject);
begin
  inherited;
  Width:= 460;
  Height:= 292;
  Window.Text:= 'Pgyer Config';
  ...
  FLblUKey := TD2Text(createD2VO(Root, TD2Text, 8, 168, 90, 32));
  FLblUKey.setup('UKey');
  FTxtUKey := TD2HudTextBox(createD2VO(Root, TD2HudTextBox, 120, 170, 300, 28));

  FLblApiKey := TD2Text(createD2VO(Root, TD2Text, 8, 200, 90, 32));
  FLblApiKey.setup('Api Key');
  FTxtApiKey := TD2HudTextBox(createD2VO(Root, TD2HudTextBox, 120, 202, 300, 28));

  // load
  FTxtProvisionKey.Text:= ioscfg.PgyerProvisionKey;
  FTxtProvisionValue.Text:= ioscfg.PgyerProvisionValue;
  FTxtCertificate.Text:= ioscfg.PgyerCertificate;
  FTxtTeamId.Text:= ioscfg.PgyerTeamId;
  FChkAutoSign.IsChecked:= ioscfg.PgyerAutoSign;
  FTxtUKey.Text:= ioscfg.PgyerUKey;
  FTxtApiKey.Text:= ioscfg.PgyerApiKey;
